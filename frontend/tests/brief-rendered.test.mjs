...
test('create adds skills and credentials together and saves minimum experience in years', async () => {
  ...
  await user.type(screen.getByLabelText('Role description'), 'Platform engineer')
  assert.equal(request, null, 'editing must not save or search')
...
test('editing the description preserves saved criteria and only saves on final confirmation', async () => {
  ...
  await user.clear(screen.getByLabelText('Role description'))
  await user.type(screen.getByLabelText('Role description'), 'Senior platform engineer')
