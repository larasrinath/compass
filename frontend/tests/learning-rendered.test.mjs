...
test('results walkthrough requires review, selection, then View comparison', async () => {
  ...
  const user=userEvent.setup({document:window.document})
  assert.ok(screen.getByRole('button',{name:'Confirm review & compare'}).disabled)
  await user.type(screen.getByRole('textbox',{name:'Inspection note'}),'Checked names and duplicate sources.')
  await user.click(screen.getByRole('button',{name:'Confirm review & compare'}))
...
test('weights use versioned number fields and verification stays local', async () => {
  ...
  await user.click(screen.getByRole('button',{name:'Save from 1'}))
  assert.ok(screen.getByRole('button',{name:'Save from 2'}))
  assert.ok(screen.getByRole('button',{name:'Accept Gate B'}).disabled)
  for (const checkbox of screen.getAllByRole('checkbox').slice(0,10)) await user.click(checkbox)
  assert.equal(screen.getByRole('button',{name:'Accept Gate B'}).disabled,false)
  await user.click(screen.getByRole('button',{name:'Accept Gate B'}))
