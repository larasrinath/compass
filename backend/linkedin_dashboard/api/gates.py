...
def _current_session_id(session: Any) -> str:
    ...
    for item in expected:
        ...
        if item.get("state") == "missing":
            if latest is not None:
                return False
            reason, error_id = _missing_reason(
                session, score.candidate_id, item["name"]
            )
            if (
                item.get("missing_reason") != reason.value
                or item.get("section_error_id") != error_id
            ):
                return False
            continue
        if (
        ...
        with database.transition_lock:
            with database.sessions.begin() as session:
                ...
                rows = session.execute(
                    ...
                    .join(
                        EvidenceSetRecord,
                        EvidenceSetRecord.id == Evidence.evidence_set_id,
                    )
                    ...
                    .join(
                        ProfileSection, ProfileSection.id == Evidence.profile_section_id
                    )
                    .where(
