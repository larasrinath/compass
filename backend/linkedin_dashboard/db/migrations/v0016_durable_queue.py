...
from linkedin_dashboard.db.models import Job, JobAttempt, QueueControl
...
TRIGGER_NAMES = (
    ...
    "job_state_transition_is_valid",
    "job_terminal_is_immutable",
    "job_continuation_lineage_insert",
    "job_continuation_lineage_update",
)
...
STATEMENTS = (
    ...
    WHEN (OLD.raw_response IS NOT NULL AND OLD.raw_response <> 'null'
      ...
      OR (OLD.raw_error IS NOT NULL AND OLD.raw_error <> 'null'
          AND NEW.raw_error IS NOT OLD.raw_error)
    BEGIN
    ...
    WHEN OLD.finished_at IS NOT NULL
     AND (NEW.id IS NOT OLD.id
       ...
       OR NEW.worker_token IS NOT OLD.worker_token
    ...
    WHEN NEW.attempts < 0 OR NEW.max_attempts < 1
      ...
      OR (NEW.state IN ('pending','queued') AND NEW.finished_at IS NOT NULL)
      OR (NEW.state = 'pending'
          AND (NEW.attempts <> 0 OR NEW.started_at IS NOT NULL))
      OR (NEW.state <> 'running' AND NEW.claim_token IS NOT NULL)
    BEGIN
    ...
    """,
    """
    CREATE TRIGGER job_state_transition_is_valid
    ...
    WHEN NOT (
      ...
      OR (OLD.state = 'running'
          AND NEW.state IN ('queued','done','failed','interrupted'))
    )
    BEGIN
    ...
    """,
    """
    CREATE TRIGGER job_terminal_is_immutable
    BEFORE UPDATE ON job
    FOR EACH ROW
    WHEN OLD.state IN ('done','failed','interrupted','cancelled')
    BEGIN
      SELECT RAISE(ABORT, 'terminal job is immutable');
    END
    """,
    """
    CREATE TRIGGER job_continuation_lineage_insert
    BEFORE INSERT ON job
    FOR EACH ROW
    WHEN json_type(NEW.payload, '$.parent_job_id') IS NOT NULL
     AND NOT EXISTS (
       SELECT 1 FROM job AS parent
        WHERE parent.id = json_extract(NEW.payload, '$.parent_job_id')
          AND parent.session_id = NEW.session_id
          AND parent.kind = 'get_person_profile'
          AND NEW.kind = 'get_person_profile'
          AND parent.state = 'done'
          AND parent.error = 'RATE_LIMIT'
     )
    BEGIN
      SELECT RAISE(ABORT, 'invalid continuation lineage');
    END
    """,
    """
    CREATE TRIGGER job_continuation_lineage_update
    BEFORE UPDATE OF session_id, kind, payload ON job
    FOR EACH ROW
    WHEN json_type(NEW.payload, '$.parent_job_id') IS NOT NULL
     AND NOT EXISTS (
       SELECT 1 FROM job AS parent
...
def apply(connection: Connection) -> None:
    job_sql = connection.exec_driver_sql(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='job'"
    ).scalar_one()
    if "'pending'" not in str(job_sql) or "claim_token" not in str(job_sql):
        if connection.exec_driver_sql("SELECT EXISTS (SELECT 1 FROM job)").scalar_one():
            raise RuntimeError(
                f"cannot apply {VERSION}: pre-queue job rows must be empty"
            )
        connection.exec_driver_sql("DROP TABLE job")
        cast(Table, Job.__table__).create(connection)
    ...
    connection.exec_driver_sql(
        "CREATE UNIQUE INDEX IF NOT EXISTS one_profile_continuation "
        "ON job (json_extract(payload, '$.parent_job_id')) "
        "WHERE kind = 'get_person_profile' "
        "AND json_type(payload, '$.parent_job_id') IS NOT NULL"
    )
