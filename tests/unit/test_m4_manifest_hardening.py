...
from linkedin_dashboard.db.models import AuditLog, BriefSkill, RoleBrief, ScoringConfig
from linkedin_dashboard.db.scoring_manifest import build_manifest, normalize
...
from linkedin_dashboard.services.scoring_service import DEFAULT_WEIGHT_MAP
...
def _manifest_with_required(term: str, aliases: list[str]) -> dict[str, object]:
    manifest = _empty_manifest()
    entry = {"display": term, "term": normalize(term), "aliases": aliases}
    manifest["S-1"] = [entry]
    manifest["S-3"] = [entry]
    return manifest
...
@pytest.mark.parametrize("recursive", ("ON", "OFF"))
def test_child_sources_reject_nul_and_hidden_tails(
    tmp_path: Path, recursive: str
) -> None:
    path = tmp_path / f"child-nul-{recursive}.db"
    database = Database(path)
    database.initialize()
    database.dispose()
    cases = (
        ("Py\x00thon", "[]"),
        ("Py\x00" + "x" * 1_000, "[]"),
        ("Python", json.dumps(["Py\x00thon"])),
        ("Python", json.dumps(["Py\x00" + "x" * 1_000])),
    )

    with _connection(path, recursive) as connection:
        for table in ("brief_skill", "brief_term", "brief_credential"):
            for index, (term, aliases) in enumerate(cases):
                suffix = f"nul-{table}-{index}"
                brief_id = _insert_unsealed_brief(connection, suffix)
                with pytest.raises(sqlite3.IntegrityError, match="source is invalid"):
                    _child_insert(connection, table, suffix, brief_id, term, aliases)
...
@pytest.mark.parametrize("recursive", ("ON", "OFF"))
@pytest.mark.parametrize("hidden_tail", (False, True))
@pytest.mark.parametrize("field", ("term", "alias", "location"))
def test_sealed_manifest_rejects_every_nul_bearing_scoring_string(
    tmp_path: Path, recursive: str, hidden_tail: bool, field: str
) -> None:
    path = tmp_path / f"manifest-nul-{field}-{recursive}-{hidden_tail}.db"
    database = Database(path)
    database.initialize()
    database.dispose()
    nul = "visible\x00" + "x" * 1_000 if hidden_tail else "\x00tail"

    with _connection(path, recursive) as connection:
        suffix = f"manifest-nul-{field}"
        _insert_session(connection, suffix)
        if field == "location":
            manifest = _empty_manifest()
            manifest["S-6"] = [{"display": nul, "term": normalize(nul), "aliases": []}]
            values = list(
                _role_values(
                    suffix,
            ...
            term = nul if field == "term" else "Python"
            aliases = [normalize(nul)] if field == "alias" else []
            manifest = _manifest_with_required(term, aliases)
            connection.execute(
                _ROLE_INSERT,
                _role_values(
                    suffix,
            ...
            trigger_sql = connection.execute(
                ...
                "AND name='brief_skill_source_shape_v27'"
            ).fetchone()[0]
            connection.execute("DROP TRIGGER brief_skill_source_shape_v27")
            connection.execute(
                "INSERT INTO brief_skill VALUES (?,?,?,?,?,0)",
                (
                    f"source-{suffix}",
                    ...
                    term,
                    "required",
                    json.dumps([nul] if field == "alias" else []),
                ),
            )
...
@pytest.mark.parametrize("recursive", ("ON", "OFF"))
def test_sealed_manifest_rejects_a_leading_nul_hidden_from_s6_source(
    tmp_path: Path, recursive: str
) -> None:
    path = tmp_path / f"leading-nul-location-{recursive}.db"
    database = Database(path)
    database.initialize()
    database.dispose()

    with _connection(path, recursive) as connection:
        suffix = f"leading-nul-{recursive}"
        _insert_session(connection, suffix)
        values = list(
            _role_values(
                suffix,
                json.dumps(_empty_manifest(), separators=(",", ":")),
                sealed=False,
            )
        )
        values[8] = "\x00" + "x" * 1_000
        connection.execute(_ROLE_INSERT, values)
        with pytest.raises(
            sqlite3.IntegrityError, match="scoring inputs are not canonical"
        ):
...
def test_manifest_builder_rejects_ambiguous_iterable_shapes(
    required_skills: object,
) -> None:
    with pytest.raises(ValueError):
        build_manifest(
            required_skills=cast(Any, required_skills),
            optional_skills=(),
            target_titles=(),
            industries=(),
            location="",
...
def test_manifest_builder_accepts_only_bounded_tuple_and_list_entries() -> None:
    entries = (("Python", ("Py",)), ["Rust", ["Ferris"]])
    expected = [
        {"display": "Python", "term": "python", "aliases": ["py"]},
        {"display": "Rust", "term": "rust", "aliases": ["ferris"]},
    ]
    sources = (entries, list(entries), (entry for entry in entries))
    for source in sources:
        manifest = build_manifest(
            required_skills=cast(Any, source),
            optional_skills=(),
            target_titles=(),
            industries=(),
            location="",
        ...
        assert manifest["S-1"] == expected


@pytest.mark.parametrize("location", (None, 7, ["Austin"]))
def test_manifest_builder_rejects_non_string_locations(location: object) -> None:
...
def test_manifest_builder_bounds_the_outer_iterable_before_materializing() -> None:
    yielded = 0
    ...
    with pytest.raises(ValueError, match="too many term entries"):
        build_manifest(
...
@pytest.mark.parametrize("field", ("term", "alias", "location"))
@pytest.mark.parametrize("hidden_tail", (False, True))
def test_manifest_builder_rejects_nul_before_length_checks(
    field: str, hidden_tail: bool
) -> None:
    nul = "safe\x00" + ("x" * 1_000 if hidden_tail else "tail")
    term = nul if field == "term" else "Python"
    aliases: tuple[str, ...] = (nul,) if field == "alias" else ()
    with pytest.raises(ValueError, match="NUL"):
        build_manifest(
            required_skills=((term, aliases),),
            optional_skills=(),
            target_titles=(),
            industries=(),
            location=nul if field == "location" else "",
            required_credentials=(),
        )


def test_duplicate_primary_terms_merge_deterministically(tmp_path: Path) -> None:
    app = create_app(Settings(db_path=tmp_path / "duplicates.db", llm_provider="null"))
    inputs = [
        {"term": "\uff30\uff39\uff34\uff28\uff2f\uff2e", "aliases": ["Py"]},
        {"term": "Python", "aliases": ["CPython"]},
    ]
    persisted: list[tuple[str, tuple[str, ...], dict[str, object]]] = []
    with TestClient(app, base_url="http://127.0.0.1") as client:
        for index, required_skills in enumerate((inputs, list(reversed(inputs)))):
            session_id = client.post(
                "/api/session", json={"label": f"duplicates-{index}"}
            ).json()["id"]
            ...
            with app.state.database.sessions() as session:
                brief = session.query(RoleBrief).filter_by(session_id=session_id).one()
                skill = session.query(BriefSkill).filter_by(brief_id=brief.id).one()
                persisted.append(
                    (skill.term, tuple(skill.aliases), dict(brief.scoring_inputs or {}))
                )
    assert persisted[0] == persisted[1]
    assert persisted[0][0:2] == ("Python", ("CPython", "Py"))


def test_alias_from_any_duplicate_primary_can_block_the_whole_brief(
    tmp_path: Path,
) -> None:
    app = create_app(
        Settings(db_path=tmp_path / "duplicate-collision.db", llm_provider="null")
    )
    with TestClient(app, base_url="http://127.0.0.1") as client:
        session_id = client.post(
            "/api/session", json={"label": "duplicate collision"}
        ).json()["id"]
        response = client.post(
            "/api/briefs",
            json={
        ...
        with app.state.database.sessions() as session:
            assert (
                session.query(RoleBrief).filter_by(session_id=session_id).count() == 0
            )


def test_weights_update_ignores_the_reserved_session_on_fresh_database(
    tmp_path: Path,
) -> None:
    app = create_app(
        Settings(db_path=tmp_path / "fresh-weights.db", llm_provider="null")
    )
    ...
    with TestClient(app, base_url="http://127.0.0.1") as client:
        ...
        with app.state.database.sessions() as session:
            before = [
                (row.id, row.version, row.superseded_at, dict(row.weights))
                for row in session.query(ScoringConfig)
                .filter_by(session_id=sentinel)
                .order_by(ScoringConfig.version)
            ]
            audits_before = [
                (row.id, row.action, dict(row.detail))
        ...
        assert response.json()["detail"] == "session does not exist"
        with app.state.database.sessions() as session:
            after = [
                (row.id, row.version, row.superseded_at, dict(row.weights))
                for row in session.query(ScoringConfig)
